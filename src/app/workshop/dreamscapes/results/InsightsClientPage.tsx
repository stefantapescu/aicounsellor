...
// Import the type from the server component (PersonalityScores is not needed here)
import type { InsightsPageData } from "./page";

// Sample data for sections other than personality radar (will use props for radar)
// TODO: Replace with actual data fetching logic passed via props if needed
const mockInsightsData = {
  ...
  // Personality traits will come from props
}

// Neural network connections for visualization
// TODO: Generate dynamically based on insightsData relationships
const connections = [
...
interface InsightsClientPageProps {
  insightsData: InsightsPageData;
}

export default function InsightsClientPage({ insightsData }: InsightsClientPageProps) {
  const [isAnalyzing, setIsAnalyzing] = useState(false)
  const [activeSection, setActiveSection] = useState("summary")
  const [showNetworkGraph, setShowNetworkGraph] = useState(false)
  const networkCanvasRef = useRef<HTMLCanvasElement>(null)
  // radarCanvasRef is no longer needed for Recharts
  ...
  const personalityDataForChart = insightsData.personalityScores
    ? Object.entries(insightsData.personalityScores)
        .map(([trait, score]) => ({
          trait: trait,
          score: score ?? 0, // Default score to 0 if undefined
          fullMark: 100, // Assuming scores are out of 100
        }))
        // Ensure only the Big Five are included if necessary
        .filter(item => ["Openness", "Conscientiousness", "Extraversion", "Agreeableness", "Neuroticism"].includes(item.trait))
    : [];

  // Draw network graph visualization (existing logic)
  useEffect(() => {
    // ... (keep existing network graph canvas logic) ...
    ...
    // Add themes as nodes
    mockInsightsData.themes.forEach((theme, i) => {
      const angle = ((Math.PI * 2) / mockInsightsData.themes.length) * i
    ...
    mockInsightsData.interests.forEach((interest, i) => {
      const angle = ((Math.PI * 2) / mockInsightsData.interests.length) * i + Math.PI / 4
    ...
    // Add values as nodes
    mockInsightsData.coreValues.forEach((value, i) => {
      const angle = ((Math.PI * 2) / mockInsightsData.coreValues.length) * i + Math.PI / 2
    ...
    mockInsightsData.motivators.forEach((motivator, i) => {
      const angle = ((Math.PI * 2) / mockInsightsData.motivators.length) * i + Math.PI / 3
  ...
  // Draw bubble chart for career paths (existing logic)
  useEffect(() => {
    // ... (keep existing bubble chart canvas logic) ...
    ...
    // Prepare bubble data
    const bubbles = mockInsightsData.careerPaths.map((path, index) => {
      const size = (path.match / 100) * 50 + 20 // Scale bubble size based on match percentage

      // Position bubbles in a circular pattern
      const angle = (Math.PI * 2 * index) / mockInsightsData.careerPaths.length
      const radius = canvas.width * 0.3
            ...
            >
              ...
              <div className="overflow-hidden rounded-3xl border border-purple-500/20 bg-black/40 backdrop-blur-md">
                <div className="p-6">
                  ...
                  <div className="relative rounded-2xl border border-purple-500/20 bg-purple-900/10 p-6">
                    ...
                    {/* Use mock data for summary */}
                    <p className="relative z-10 text-purple-200">{mockInsightsData.summary}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="overflow-hidden rounded-3xl border border-purple-500/20 bg-black/40 backdrop-blur-md">
                <div className="p-6">
                  ...
                  <div className="space-y-4">
                    {/* Use mock data */}
                    {mockInsightsData.coreValues.map((value, index) => (
                      <motion.div
                        key={index}
                        initial={{ width: 0 }}
              ...
              <div className="overflow-hidden rounded-3xl border border-purple-500/20 bg-black/40 backdrop-blur-md">
                <div className="p-6">
                  ...
                  <div className="space-y-4">
                     {/* Use mock data */}
                    {mockInsightsData.themes.map((theme, index) => (
                      <motion.div
                        key={index}
                        initial={{ width: 0 }}
              ...
              <div className="overflow-hidden rounded-3xl border border-purple-500/20 bg-black/40 backdrop-blur-md">
                <div className="p-6">
                  ...
                  <div className="space-y-4">
                     {/* Use mock data */}
                    {mockInsightsData.motivators.map((motivator, index) => (
                      <motion.div
                        key={index}
                        initial={{ width: 0 }}
            ...
            >
              <div className="p-6">
                ...
                <div className="relative mx-auto aspect-square max-w-2xl">
                  {/* Replace Canvas with Recharts Radar Chart */}
                  {personalityDataForChart.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart cx="50%" cy="50%" outerRadius="80%" data={personalityDataForChart}>
                        <defs>
                        ...
                        <PolarAngleAxis dataKey="trait" stroke="rgba(255, 255, 255, 0.7)" />
                        <PolarRadiusAxis angle={30} domain={[0, 100]} tick={false} axisLine={false} />
                        <Radar name="Personality" dataKey="score" stroke="#EC4899" fill="url(#radarGradient)" fillOpacity={0.6} />
                        <RechartsTooltip
                          contentStyle={{ backgroundColor: 'rgba(0,0,0,0.8)', border: '1px solid rgba(139, 92, 246, 0.5)', borderRadius: '8px' }}
                          itemStyle={{ color: '#ddd' }}
                    ...
                    <div className="flex h-full items-center justify-center text-purple-300">
                      Personality data not available. Complete the assessment games!
                    </div>
                  )}
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-5">
                  {/* Display scores below chart */}
                  {personalityDataForChart.map((traitData, index) => (
                    <div
                      key={index}
                      className="rounded-xl border border-purple-500/20 bg-purple-900/10 p-3 text-center"
                    >
                      <div className="font-medium text-white">{traitData.trait}</div>
                      <div className="text-sm text-purple-300">{traitData.score}%</div>
                    </div>
                  ))}
                </div>
              </div>
